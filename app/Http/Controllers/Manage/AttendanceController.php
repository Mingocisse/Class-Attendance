...
class AttendanceController extends BaseController
{
    ...
    public function store(StoreAttendanceRequest $request){
        ...
        $attendance = Attendance::create([
            'user_id' => Auth::id(),
            'subject_id' => $subject->id,  // Explicitly set from validated subject
            'date' => now(),
            ]);
        $subject = Subject::findorfail($request->get('subject_id'));
    ...
    public function show(Attendance $attendance)
    {
        ...
        $total_present = $attendance->students->where('pivot.status', 1)->count();
        $total_absent = $attendance->students->where('pivot.status', 0)->count();
    
        return view('Manage.pages.Attendance.show', compact('attendance', 'total_present', 'total_absent'));
    }
    /**
